...
function filtrarnotacreditodeventa(){
...
var url = '/notacreditoventacab_filtro_lst/?' + params;
...
function notacreditoventacab_listar(){
...
var url = "/notacreditoventacab/" + variable + "/listar/";  // Construir la URL de la vista
...
function eliminar_notacreditondeventacab(pk) {
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/notacreditoventacab/' + pk + '/eliminar/', { method: 'DELETE', headers: headers  })
                    .then((response) => {
                    if (response.ok) {
                        ...
                        Swal.fire({  title: '',  text: 'eliminado exitosamente ',
                         ...
                         document.location.href= "/notacreditoventacab/crear/";
...
if (document.querySelector('#notacredventa-list')) {
busdetalle();
cargarcmbart();
cargarcombdep();
...
$.ajax({
     url: "/notacreditoventadet_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
...
for (var i = 0; i < list.length; i++) {
      ...
      var notacreditoventadet = list[i];
      lin=lin+1;   e=i+1;
      col='#D5F5E3';
      ...
      html += '<div class ="campcantiva"  > <input   id="orden'+ e +'"  value="'+ notacreditoventadet.orden + '"</input> </div>';
      html += '<div class ="camplst" > <input id="codigo'+ e +'" value="'+ notacreditoventadet.codigo + '"</input> </div>';
      html += '<div class ="campdescr" > <input id="descripcion'+ e +'"  list="descrip_list"  value=" '+ notacreditoventadet.descripcion + '"</input> </div>';
      html += '</div> ';
      html += '<div id= "lin'+ lin +'" class ="detallelinea2"> ';
      html += '<div class ="campcantiva" > <input id="cantidad'+ e +'" value=" '+ number_format(notacreditoventadet.cantidad,3) + '"</input> </div>';
      html += '<div class ="camplst" > <input id="precio'+ e +'" value=" '+ number_format(notacreditoventadet.precio,0) + '"</input> </div>';
      html += '<div class ="campcantiva" > <input id="iva'+ e +'" value=" '+ notacreditoventadet.iva + '"</input> </div>';
      html += '<div class ="camplst" > <input id="subtotal'+ e +'" value=" '+ number_format(notacreditoventadet.subtotal,0) + '"</input> </div>';
      html += '<div class ="campbtn" > <button  class="btnlstdet" onClick="editarnotacreditoventadet('+ notacreditoventadet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlstdet" onClick="notacreditoventadet_eliminar(\''+notacreditoventadet.pkncd +'\' )" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
      total=total+(notacreditoventadet.cantidad*notacreditoventadet.precio);

}
if (total>0){html +=sumatotal(total);}
 html +=listadoagregardetalle(lin,e);

$('#notacredventa-list').html(html);
...
}else{html=listadocab(); $('#notacredventa-list').html(html);}
...
function editarnotacreditoventadet(orden){
...
if (document.getElementById('labtitdetmod').textContent==="INGRESAR-DETALLE") {notacreditoventadet_guardar(); }
if (document.getElementById('labtitdetmod').textContent==="EDITAR-DETALLE") {notacreditoventadet_editar(); }
var ob="#btnguardardet";  $(''+ ob +'').prop('disabled', false);
}


function notacreditoventadet_editar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/notacreditoventadet_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
...
function notacreditoventadet_eliminar(pk){
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/notacreditoventadet/' + pk + '/eliminar/',
                    {
                        method: 'DELETE',
...
function notacreditoventadet_guardar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/notacreditoventadet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
