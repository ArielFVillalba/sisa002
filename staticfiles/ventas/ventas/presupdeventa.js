...
function filtrarpresupuestodeventa(){
...
var url = '/presupuestoventacab_filtro_lst/?' + params;
...
function buscarpresupuestodeventa(){
...
var url = "/presupuestoventacab/" + variable + "/listar/";  // Construir la URL de la vista
...
function eliminar_presupuestodeventacab(pk) {
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/presupuestoventacab/' + pk + '/eliminar/', { method: 'DELETE', headers: headers  })
                    .then((response) => {
                    if (response.ok) {
                        ...
                        Swal.fire({  title: '',  text: 'eliminado exitosamente ',
                         ...
                         document.location.href= "/presupuestoventacab/crear/";
...
function busdetalle(){
var pkpv = document.getElementById('id_pkpv').value;
if (pkpv.length>10){
    orden=0;
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/presupuestoventadet_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pkpv': pkpv,'orden': orden},
...
for (var i = 0; i < list.length; i++) {
      ...
      var presupuestoventadet = list[i];
      lin=lin+1;   e=i+1;
       col='#D5F5E3';
      ...
      html += '<div class ="campcantiva"  > <input   id="orden'+ e +'"  value="'+ presupuestoventadet.orden + '"</input> </div>';
      html += '<div class ="camplst" > <input id="codigo'+ e +'" value="'+ presupuestoventadet.codigo + '"</input> </div>';
      html += '<div class ="campdescr" > <input id="descripcion'+ e +'"  list="descrip_list"  value=" '+ presupuestoventadet.descripcion + '"</input> </div>';
      html += '</div> ';
      html += '<div id= "lin'+ lin +'" class ="detallelinea2"> ';
      html += '<div class ="campcantiva" > <input id="cantidad'+ e +'" value=" '+ number_format(presupuestoventadet.cantidad,3) + '"</input> </div>';
      html += '<div class ="camplst" > <input id="precio'+ e +'" value=" '+ number_format(presupuestoventadet.precio,0) + '"</input> </div>';
      html += '<div class ="campcantiva" > <input id="iva'+ e +'" value=" '+ presupuestoventadet.iva + '"</input> </div>';
      html += '<div class ="camplst" > <input id="subtotal'+ e +'" value=" '+ number_format(presupuestoventadet.subtotal,0) + '"</input> </div>';
      html += '<div class ="campbtn" > <button  class="btnlstdet" onClick="editarpresupuestoventadet('+ presupuestoventadet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlstdet" onClick="presupuestoventadet_eliminar(\''+presupuestoventadet.pkpvd +'\' )" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
      total=total+(presupuestoventadet.cantidad*presupuestoventadet.precio);
...
}else{html=listadocab(); $('#presup-list').html(html);}
...
function listadoagregardetalle(lin,e){
        ...
        lin=lin+1;   e=e+1;  col='#D5F5E3';
            col='#D5F5E3';
      if (e > 1 ){ if (e % 2 === 0) { col='#FCF3CF';   } else {  col='#D5F5E3';   }  }      html += '<div id= "lin0'+ lin +'" class ="detallelinea0"';
...
function presupuestoventadet_eliminar(pk){
 // Obtener el token CSRF de la cookie
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/presupuestoventadet/' + pk + '/eliminar/',
                    {
                        method: 'DELETE',
...
function editarpresupuestoventadet(orden){
...
if (document.getElementById('labtitdetmod').textContent==="INGRESAR-DETALLE") {presupuestoventadet_guardar(); }
if (document.getElementById('labtitdetmod').textContent==="EDITAR-DETALLE") {presupuestoventadet_editar(); }
var ob="#btnguardardet";  $(''+ ob +'').prop('disabled', false);
}


function presupuestoventadet_editar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/presupuestoventadet_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pkpv': pkpv,  'orden':orden,'codigo':codigo,'cantidad':cantidad,'precio':precio },
    success: function(data) {
 if (data.success) {
...
function presupuestoventadet_guardar(){
var pkpv = document.getElementById('id_pkpv').value;
var codigo = document.getElementById('codigo').value;
var cantidad = document.getElementById('cantidad').value;
var precio = document.getElementById('precio').value;
...
$.ajax({
     url: "/presupuestoventadet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pkpv': pkpv, 'codigo':codigo,'cantidad':cantidad,'precio':precio },
     success: function(data) {
    if (data.success) {
        Swal.fire({  title: '',  text: 'ACTUALIZADO',
        icon: 'success',  customClass: {container: 'msnsuccess', }});
