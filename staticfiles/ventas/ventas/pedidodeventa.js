...
var url = '/pedidoventacab_filtro_lst/?' + params;
...
function buscarpedidodeventara(){
...
var url = "/pedidoventacab/" + variable + "/listar/";  // Construir la URL de la vista
...
function eliminar_pedidodeventacab(pk) {
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/pedidoventacab/' + pk + '/eliminar/', { method: 'DELETE', headers: headers  })
                    .then((response) => {
                    if (response.ok) {
                        ...
                        Swal.fire({  title: '',  text: 'eliminado exitosamente ',
                         ...
                         document.location.href= "/pedidoventacab/crear/";
...
$.ajax({
     url: "/pedidoventadet_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pkpv': pkpv,'orden': orden},
     success: function(data) {
     if (data.success) {
     var list = data.datos;       var html = '';  lin=0;  e=0;
    total=0;

for (var i = 0; i < list.length; i++) {
      if (i === 0){  html=listadocab();  }
      var pedidoventadet = list[i];
      ...
      html += '<div class ="campcantiva"  > <input   id="orden'+ e +'"  value="'+ pedidoventadet.orden + '"</input> </div>';
      html += '<div class ="camplst" > <input id="codigo'+ e +'" value="'+ pedidoventadet.codigo + '"</input> </div>';
      html += '<div class ="campdescr" > <input id="descripcion'+ e +'"  list="descrip_list"  value=" '+ pedidoventadet.descripcion + '"</input> </div>';
      html += '</div> ';
      html += '<div id= "lin'+ lin +'" class ="detallelinea2"> ';
      html += '<div class ="campcantiva" > <input id="cantidad'+ e +'" value=" '+ number_format(pedidoventadet.cantidad,3) + '"</input> </div>';
      html += '<div class ="camplst" > <input id="precio'+ e +'" value=" '+ number_format(pedidoventadet.precio,0) + '"</input> </div>';
      html += '<div class ="campcantiva" > <input id="iva'+ e +'" value=" '+ pedidoventadet.iva + '"</input> </div>';
      html += '<div class ="camplst" > <input id="subtotal'+ e +'" value=" '+ number_format(pedidoventadet.subtotal,0) + '"</input> </div>';
      html += '<div class ="campbtn" > <button  class="btnlstdet" onClick="editarpedidoventadet('+ pedidoventadet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlstdet" onClick="pedidoventadet_eliminar(\''+pedidoventadet.pkpvd +'\' )" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
      total=total+(pedidoventadet.cantidad*pedidoventadet.precio);

}
if (total>0){html +=sumatotal(total);}
 html +=listadoagregardetalle(lin,e);

$('#pedidoventas-list').html(html);
...
setTimeout(actventaelem,500);
}else{html=listadocab(); $('#pedidoventas-list').html(html);}
...
function pedidoventadet_eliminar(pk){
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/pedidoventadet/' + pk + '/eliminar/',
                    {
                        method: 'DELETE',
...
function editarpedidoventadet(orden){
...
if (document.getElementById('labtitdetmod').textContent==="INGRESAR-DETALLE") {pedidoventaradet_guardar(); }
if (document.getElementById('labtitdetmod').textContent==="EDITAR-DETALLE") {pedidoventaradet_editar(); }
var ob="#btnguardardet";  $(''+ ob +'').prop('disabled', false);
}


function pedidoventaradet_editar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/pedidoventadet_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
...
function pedidoventaradet_guardar(){
var pkpv = document.getElementById('id_pkpv').value;
var codigo = document.getElementById('codigo').value;
var cantidad = document.getElementById('cantidad').value;
var precio = document.getElementById('precio').value;
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/pedidoventadet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
