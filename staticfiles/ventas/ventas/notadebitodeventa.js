...
function filtrarnotadebitodeventa(){
...
var url = '/notadebitoventacab_filtro_lst/?' + params;
...
function buscarnotadebitodeventa(){
...
var url = "/notadebitoventacab/" + variable + "/listar/";  // Construir la URL de la vista
...
function eliminar_notadebitondeventacab(pk) {
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/notadebitoventacab/' + pk + '/eliminar/', { method: 'DELETE', headers: headers  })
                    .then((response) => {
                    if (response.ok) {
                        ...
                        Swal.fire({  title: '',  text: 'eliminado exitosamente ',
                         ...
                         document.location.href= "/notadebitoventacab/crear/";
...
if (document.querySelector('#notadevventa-list')) {
busdetalle();
cargarcmbart();
}
}
...
$.ajax({
     url: "/notadebitoventadet_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pknd': pknd,'orden': orden},
...
for (var i = 0; i < list.length; i++) {
      ...
      html += '<div class ="campcantiva"  > <input   id="orden'+ e +'"  value="'+ notadebitoventadet.orden + '"</input> </div>';
      html += '<div class ="camplst" > <input id="codigo'+ e +'" value="'+ notadebitoventadet.codigo + '"</input> </div>';
      html += '<div class ="campdescr" > <input id="descripcion'+ e +'"  list="descrip_list"  value=" '+ notadebitoventadet.descripcion + '"</input> </div>';
      html += '</div> ';
      html += '<div id= "lin'+ lin +'" class ="detallelinea2"> ';
      html += '<div class ="campcantiva" > <input id="cantidad'+ e +'" value=" '+ number_format(notadebitoventadet.cantidad,3) + '"</input> </div>';
      html += '<div class ="camplst" > <input id="precio'+ e +'" value=" '+ number_format(notadebitoventadet.precio,0) + '"</input> </div>';
      html += '<div class ="campcantiva" > <input id="iva'+ e +'" value=" '+ notadebitoventadet.iva + '"</input> </div>';
      html += '<div class ="camplst" > <input id="subtotal'+ e +'" value=" '+ number_format(notadebitoventadet.subtotal,0) + '"</input> </div>';
      html += '<div class ="campbtn" > <button  class="btnlstdet" onClick="editarnotadebitoventadet('+ notadebitoventadet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlstdet" onClick="notadebitoventadet_eliminar(\''+notadebitoventadet.pkndd +'\' )" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
      total=total+(notadebitoventadet.cantidad*notadebitoventadet.precio);

}
if (total>0){html +=sumatotal(total);}
 html +=listadoagregardetalle(lin,e);

$('#notadevventa-list').html(html);
...
setTimeout(actcompelem,500);
}else{html=listadocab(); $('#notadevventa-list').html(html);}
...
function notadebitoventadet_eliminar(pk){
  // Obtener el token CSRF de la cookie
  const csrftoken = document.cookie.match(/csrftoken=([\w-]+)/)[1];
  // Configurar el encabezado X-CSRFToken en la solicitud DELETE
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/notadebitoventadet/' + pk + '/eliminar/', { method: 'DELETE', headers: headers  })
                    .then((response) => {
                    if (response.ok) {
                        // Si la eliminación fue exitosa, mostrar un mensaje de confirmación
                        Swal.fire({  title: '',  text: 'eliminado exitosamente ',
...
function editarnotadebitoventadet(orden){
...
if (document.getElementById('labtitdetmod').textContent==="INGRESAR-DETALLE") {notadebitoventadet_guardar(); }
if (document.getElementById('labtitdetmod').textContent==="EDITAR-DETALLE") {notadebitoventadet_editar(); }
var ob="#btnguardardet";  $(''+ ob +'').prop('disabled', false);
}


function notadebitoventadet_editar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/notadebitoventadet_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
...
function notadebitoventadet_guardar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/notadebitoventadet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
