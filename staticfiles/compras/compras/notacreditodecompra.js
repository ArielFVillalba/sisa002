...
function filtrarnotacreditodecompra(){
...
var url = '/notacreditocompcab_filtro_lst/?' + params;
...
function buscarnotacreditodecompra(){
...
var url = "/notacreditocompcab/" + variable + "/listar/";  // Construir la URL de la vista
...
function eliminar_notacreditondecompracab(pk) {
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/notacreditocompcab/' + pk + '/eliminar/', { method: 'DELETE', headers: headers  })
                    .then((response) => {
                    if (response.ok) {
                        ...
                        Swal.fire({  title: '',  text: 'eliminado exitosamente ',
                         ...
                         document.location.href= "/notacreditocompcab/crear/";
...
if (document.querySelector('#notacreditondecompra_frm')) {busdetalle();}
cargarcmbart();
cargarcombdep();
}
...
function busdetalle(){
var pknc = document.getElementById('id_pknc').value;
if (pknc.length>10){
    orden=0;
var csrftoken = $("[name=csrfmiddlewaretoken]").val();

$.ajax({
     url: "/notacreditocompdet_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pknc': pknc,'orden': orden},
...
for (var i = 0; i < list.length; i++) {
      ...
      html += '<div class ="campcantiva"  > <input   id="orden'+ e +'"  value="'+ notacreditocompdet.orden + '"</input> </div>';
      html += '<div class ="camplst" > <input id="codigo'+ e +'" value="'+ notacreditocompdet.codigo + '"</input> </div>';
      html += '<div class ="campdescr" > <input id="descripcion'+ e +'"  list="descrip_list"  value=" '+ notacreditocompdet.descripcion + '"</input> </div>';
      html += '</div> ';
      html += '<div id= "lin'+ lin +'" class ="detallelinea2"> ';
      html += '<div class ="campcantiva" > <input id="cantidad'+ e +'" value=" '+ number_format(notacreditocompdet.cantidad,3) + '"</input> </div>';
      html += '<div class ="camplst" > <input id="precio'+ e +'" value=" '+ number_format(notacreditocompdet.precio,0) + '"</input> </div>';
      html += '<div class ="campcantiva" > <input id="iva'+ e +'" value=" '+ notacreditocompdet.iva + '"</input> </div>';
      html += '<div class ="camplst" > <input id="subtotal'+ e +'" value=" '+number_format(notacreditocompdet.subtotal,0) + '"</input> </div>';
      html += '<div class ="campbtn" > <button  class="btnlstdet" onClick="editarnotacreditocompdet('+ notacreditocompdet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlstdet" onClick="notacreditocompdet_eliminar(\''+notacreditocompdet.pkncd+'\')" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
      total=total+(notacreditocompdet.cantidad*notacreditocompdet.precio);
...
function listadoagregardetalle(lin,e){
      ...
      col='#D5F5E3';
      if (e > 1 ){ if (e % 2 === 0) { col='#E6FCFC';   } else {  col='#D5F5E3';   }  }      html += '<div id= "lin0'+ lin +'" class ="detallelinea0"';
...
function notacreditocompdet_eliminar(pk){
 // Obtener el token CSRF de la cookie
...
function editarnotacreditocompdet(orden){
...
if (document.getElementById('labtitdetmod').textContent==="INGRESAR-DETALLE") {notacreditocompradet_guardar(); }
if (document.getElementById('labtitdetmod').textContent==="EDITAR-DETALLE") {notacreditocompradet_editar(); }
var ob="#btnguardardet";  $(''+ ob +'').prop('disabled', false);

}


function notacreditocompradet_editar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/notacreditocompdet_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pknc': pknc,  'orden':orden,'codigo':codigo,'cantidad':cantidad,'precio':precio,'deposito':deposito },
    success: function(data) {
    if (data.success) {
...
function notacreditocompradet_guardar(){
var pknc = document.getElementById('id_pknc').value;
var codigo = document.getElementById('codigo').value;
var deposito = document.getElementById('id_deposito').value;
var cantidad = document.getElementById('cantidad').value;
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/notacreditocompdet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pknc': pknc, 'codigo':codigo,'cantidad':cantidad,'precio':precio ,'deposito':deposito },
     success: function(data) {
