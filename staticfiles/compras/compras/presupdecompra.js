...
function filtrarpresupuestodecompra(){
...
var url = '/presupuestocompcab_filtro_lst/?' + params;
...
function buscarpresupuestodecompra(){
...
var url = "/presupuestocompcab/" + variable + "/listar/";  // Construir la URL de la vista
...
function eliminar_presupuestodecompracab(pk) {
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/presupuestocompcab/' + pk + '/eliminar/', { method: 'DELETE', headers: headers  })
                    .then((response) => {
                    if (response.ok) {
                        ...
                        Swal.fire({  title: '',  text: 'eliminado exitosamente ',
                         ...
                         document.location.href= "/presupuestocompcab/crear/";
...
cargarcmbart();
if (document.querySelector('#presupuestodecompra_frm')) { busdetalle();}
...
$.ajax({
     url: "/presupuestocompdet_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
...
for (var i = 0; i < list.length; i++) {
      ...
      html += '<div class ="campcantiva"  > <input   id="orden'+ e +'"  value="'+ presupuestocompdet.orden + '"</input> </div>';
      html += '<div class ="camplst" > <input id="codigo'+ e +'" value="'+ presupuestocompdet.codigo + '"</input> </div>';
      html += '<div class ="campdescr" > <input id="descripcion'+ e +'"  list="descrip_list"  value=" '+ presupuestocompdet.descripcion + '"</input> </div>';
      html += '</div> ';
      html += '<div id= "lin'+ lin +'" class ="detallelinea2"> ';
      html += '<div class ="campcantiva" > <input id="cantidad'+ e +'" value=" '+ number_format(presupuestocompdet.cantidad,3) + '"</input> </div>';
      html += '<div class ="camplst" > <input id="precio'+ e +'" value=" '+ number_format(presupuestocompdet.precio,0) + '"</input> </div>';
      html += '<div class ="campcantiva" > <input id="iva'+ e +'" value=" '+ presupuestocompdet.iva + '"</input> </div>';
      html += '<div class ="camplst" > <input id="subtotal'+ e +'" value=" '+number_format(presupuestocompdet.subtotal,0) + '"</input> </div>';
      html += '<div class ="campbtn" > <button  class="btnlstdet" onClick="editarpresupuestocompdet('+ presupuestocompdet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlstdet" onClick="presupuestocompdet_eliminar(\''+presupuestocompdet.pkpcd +'\')" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
      total=total+(presupuestocompdet.cantidad*presupuestocompdet.precio);
...
function presupuestocompdet_eliminar(pk){
 // Obtener el token CSRF de la cookie
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/presupuestocompdet/' + pk + '/eliminar/',
                    {
                        method: 'DELETE',
...
function editarpresupuestocompdet(orden){
...
if (document.getElementById('labtitdetmod').textContent==="INGRESAR-DETALLE") {presupuestocompradet_guardar(); }
if (document.getElementById('labtitdetmod').textContent==="EDITAR-DETALLE") {presupuestocompradet_editar(); }
var ob="#btnguardardet";  $(''+ ob +'').prop('disabled', false);
}


function presupuestocompradet_editar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/presupuestocompdet_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
...
function presupuestocompradet_guardar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/presupuestocompdet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
