...
if ($.fn.DataTable.isDataTable('#lstcajacierre')) {
    $('#lstcajacierre').DataTable().destroy();
}
...
var nrocaja = document.getElementById('id_nrocaja').value;
...
pag="";
if (tipolist=="general"){
if (web=="pdf"){ pag="/cierrecajacabinfpdf/"; }
if (web=="excel"){ pag="/cierrecajacabinfexcel/"; }
if (web=="csv"){ pag="/cierrecajacabinfcsv/"; }
if (web=="imp"){ pag="/compracabinfimp/"; }
};
if (tipolist=="detallado"){
if (web=="pdf"){ pag="/compradetinfpdf/"; }
if (web=="excel"){ pag="/compradetinfexcel/"; }
if (web=="csv"){ pag="/compradetinfcsv/"; }
if (web=="imp"){ pag="/compracabinfimp/"; }
};
...
function listadocajacierre(web){
if (web=="web"){
...
if (web=="web" && tipolist=="general"){ pag="/cajacierreinf_listar/"; }
if (web=="web" && tipolist=="detallado"){ pag="//"; }
...
$.ajax({
     ...
     data: { 'fechaini': fechaini,'fechafin': fechafin ,'nrocaja': nrocaja },
     success: function(data) {
     var list = data.datos;
     if (web=="web" && tipolist=="general"){ listarcierrecaja(list);   }

}});
}
}

function listarcierrecaja(list){
lin=0;
e=0;
    var html = '';
    if ($.fn.DataTable.isDataTable('#lstcajacierre')) {
...
html+=' <thead>  <tr> ';
html+=' <th>fecha</th> <th>factura ini</th> <th>factura fin </th> <th>apertura</th><th>total</th>  ';
html+=' </tr> </thead>   ';
html+=' <tbody>  ';
...
var ob = list[i];
html+=' <tr> ';
html+=" <th>"+ ob.fecha  +"</th> <th>"+ ob.facturaini +"</th>";
html+=' <th>'+ ob.facturafin +'</th><th>'+number_format(ob.apertura,0) +'</th><th>'+ number_format(ob.totalfactura,0) +'</th>';
html+=' </tr>   ';
}
html+='</tbody>   ';

$('#lstcajacierre').html(html);
$('#lstcajacierre').DataTable({
lengthMenu:[20],
searching: false,
columnDefs:[
    {orderable: false, target:[1,3,4]},
    {width: "15%", target:[0,1,2,3,4]},
],
...
html+=' <thead>  <tr> ';
html+=' <th>fecha</th> <th>factura</th> <th>proveedor</th> <th>orden</th> <th>codigo </th> ';
...
html+=" <th>"+ compra.fecha  +"</th> <th>"+ compra.nrofactura +"</th>";
...
$('#compralst th').css({'font-size':'12px'});
$('#compralst thead th').css({'font-size':'12'});
