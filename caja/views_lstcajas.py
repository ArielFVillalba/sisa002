...
def cajacierreinf_listar(request):
    if not request.user.is_authenticated:
        return redirect('login')

    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    nrocaja = request.POST['nrocaja']
    ...
    cajaapertc = cajaapertcierre.objects.filter(idempresa=didempresa, idsucursal=didsucursal)


    if cajaapertc.exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        cajaapertc = cajaapertc.filter(fecha__gte=fecha_inicio,idempresa=didempresa,idsucursal=didsucursal)
    if cajaapertc.exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        cajaapertc = cajaapertc.filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    ...
    for objeto in cajaapertc:
        ...
        'facturaini':  objeto.facturaini,
        'facturafin':  objeto.facturafin,
        'totalfactura':  objeto.totalfactura,
        'timbrado':  objeto.timbrado,
        'iddocumentos':  objeto.iddocumentos,
        'nrocaja':  objeto.nrocaja,
...
def cajacierreinf(request):
    if not request.user.is_authenticated:
        return redirect('login')
    return render(request, 'caja/lstcajacierre.html')


def cierrecajacabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    # -----------------------------------------------
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    nrocaja = request.GET.get('nrocaja', '')
    ...
    writer.writerow(["FECHA", "FACTURA INI ", "FACTURA FIN ", "APERTURA","TOTAL FACT"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.facturaini, fila.facturafin, fila.apertura, fila.totalfactura])
    return response


def compradetinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    if compracab.exists() and int(idproveedor) >= 1:
        compracab = compracab.filter(idproveedor_id=idproveedor,idempresa=didempresa,idsucursal=didsucursal)
    if compracab.exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        compracab = compracab.filter(fecha__gte=fecha_inicio,idempresa=didempresa,idsucursal=didsucursal)
    if compracab.exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        compracab = compracab.filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    if compracab.exists() and tipodoc:
        compracab = compracab.filter(tipodoc=tipodoc,idempresa=didempresa,idsucursal=didsucursal)
    ...
    total=0
    for objetocab in compracab:
        idicompra = objetocab.idcompracab
        compradet = Compradet.objects.filter(idcompracab=idicompra).order_by('orden')
...
def cierrecajacabinfexcel(request):
    if not request.user.is_authenticated:
        return redirect('login')
    # -----------------------------------------------
    fechaini = request.GET.get('fechaini', '')
    ...
    if cajaapertc.exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        cajaapertc = cajaapertc.filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    if cajaapertc.exists() and nrocaja:
        cajaapertc = cajaapertc.filter(nrocaja=nrocaja,idempresa=didempresa,idsucursal=didsucursal)
    ...
    sheet = wb.active
    sheet.title = "LISTADO  CAJAS"

    # Agrega encabezados a la hoja de cálculo
    # headers = ['Campo1', 'Campo2', 'Campo3']
    headers=["FECHA", "FACTURA INI", "FACTURA FIN", "APERTURA","TOTAL FACTURA"]
    ...
    for row_num, fila in enumerate(data, 3):
        ...
        sheet.cell(row=row_num, column=2, value=fila.facturaini)
        sheet.cell(row=row_num, column=3, value=fila.facturafin)
        sheet.cell(row=row_num, column=4, value=fila.apertura)
        total_cell = sheet.cell(row=row_num, column=5, value=fila.totalfactura)
...
def compradetinfexcel(request):
    ...
    inicio_celda.alignment = Alignment(horizontal='center')
    # Establece el valor del título general en la celda fusionada
    titulo_general = "LISTADO COMPRAS DETALLADO  "
    inicio_celda.value = titulo_general
    row_num=3
    total=0
    for objetocab in compracab:
        ...
        for objetodet in compradet:
            ...
            sheet.cell(row=row_num, column=2, value=objetocab.nrofactura)
...
def cierrecajacabinfpdf(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    if cajaapertc.exists() and fechafin:
        ...
        cajaapertc = cajaapertc.filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    if cajaapertc.exists() and nrocaja:
        cajaapertc = cajaapertc.filter(nrocaja=nrocaja)
    ...
    cajacierrecabinfpdftit(c,680,0)
    cajacierrecabinfpdfdet(cajaapertc,c,620)
    c.save()
    buf.seek(0)
...
def cajacierrecabinfpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME CAJAS ")
        cajacierrecabinfpdfcabdet(c,620)


def cajacierrecabinfpdfcabdet(c,y):
    data = [["FECHA", "FACTURA INI", "FACTURA FIN", "APERTURA","TOTAL"]]
    ...
    ancho_col = [80, 100, 100, 100, 100]  # Ancho de cada columna de la tabla 100 3.5 cm 20 7 cm
...
def cajacierrecabinfpdfdet(cajacrt,c,y):
        total=0
        detmax=25
        nropag=1
        ...
        for objeto in objetos:
                ...
                str(objeto.facturaini),
                str(objeto.facturafin),
                str(objeto.apertura),
                objeto.totalfactura
                ]
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                cajacierrecabinfpdftit(c, 680, nropag)
                fila_y=620
...
def compradetinf_listar(request):
    ...
    for objetocab in compracab:
        ...
        for objetodet in compradet:
            ...
            'nrofactura': objetocab.nrofactura,
...
def compradetinfpdf(request):
    ...
    compradetinfpdftit(c,680,0)
    compradetinfpdfdet(compracab,c,620)
    c.save()
    buf.seek(0)
...
def compradetinfpdftit(c,y,npag):
        ahora = time.strftime("%c")
        fechahora = time.strftime("%c")
        fecha = time.strftime("%x")
        print(npag)
        ...
        c.setFillColor(colors.black)
        c.setFont("Helvetica", 22)  # Puedes ajustar la fuente y el tamaño según tus necesidades
        c.drawString(lx + 20, hi-20,"INFORME COMPRA DETALLE ")
        compradetinfcabdet(c,620)


def compradetinfcabdet(c,y):
...
def compradetinfpdfdet(compracab,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir coordenadas y dimensiones de la tabla
        ...
        for objetocab in compracab:
            ...
            if compradet.exists():
                ...
                fila_x=fila_x+50
                celda(c, objetocab.nrofactura, fila_x, fila_y, 10, alto_fila,"t", "l", 20, 0)
                fila_x=fila_x+60
                ...
                for i, fila in enumerate(data):
                    ...
                    if e > detmax:
                        ...
                        nropag = nropag+1
                        e=0
                        compradetinfpdftit(c, 680, nropag)
                        fila_y=620
