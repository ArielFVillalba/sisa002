...
for (var i = 0; i < list.length; i++) {
      ...
      html += '<div class ="campbtn" > <button  class="btnlstm" onClick="editarventadet('+ ventadet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlste" onClick="ventadet_eliminar(\''+ ventadet.pkfd +'\')" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
...
}else{html=listadocab(); $('#venta-list').html(html);}
}
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/ventadet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
