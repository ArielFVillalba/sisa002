...
var url = '/ordenventacab_filtro_lst/?' + params;
...
var url = "/ordenventacab/" + variable + "/listar/";  // Construir la URL de la vista
...
function eliminar_ordendeventacab(pk) {
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/ordenventacab/' + pk + '/eliminar/',
                    {
                        method: 'DELETE',
...
var pkov = document.getElementById('id_pkov').value;
if (pkov.length>10){
    orden=0;
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/ordenventadet_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pkov': pkov,'orden': orden},
...
for (var i = 0; i < list.length; i++) {
      ...
      html += '<div class ="campcantiva"  > <input   id="orden'+ e +'"  value="'+ ordenventadet.orden + '"</input> </div>';
      html += '<div class ="camplst" > <input id="codigo'+ e +'" value="'+ ordenventadet.codigo + '"</input> </div>';
      html += '<div class ="campdescr" > <input id="descripcion'+ e +'"  list="descrip_list"  value=" '+ ordenventadet.descripcion + '"</input> </div>';
      html += '</div> ';
      html += '<div id= "lin'+ lin +'" class ="detallelinea2"> ';
      html += '<div class ="campcantiva" > <input id="cantidad'+ e +'" value=" '+ number_format(ordenventadet.cantidad,3) + '"</input> </div>';
      html += '<div class ="camplst" > <input id="precio'+ e +'" value=" '+ number_format(ordenventadet.precio,0) + '"</input> </div>';
      html += '<div class ="campcantiva" > <input id="iva'+ e +'" value=" '+ ordenventadet.iva + '"</input> </div>';
      html += '<div class ="camplst" > <input id="subtotal'+ e +'" value=" '+ number_format(ordenventadet.subtotal,0) + '"</input> </div>';
      html += '<div class ="campbtn" > <button  class="btnlstdet" onClick="editarordenventaadet('+ ordenventadet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlstdet" onClick="ordenvtadet_eliminar(\''+ordenventadet.pkovd +'\' )" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
      total=total+(ordenventadet.cantidad*ordenventadet.precio);

}
if (total>0){html +=sumatotal(total);}
 html +=listadoagregardetalle(lin,e);

$('#ordenventa-list').html(html);
...
setTimeout(actventaelem,500);
}else{html=listadocab(); $('#ordenventa-list').html(html);}
...
function ordenvtadet_eliminar(pk){
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/ordenventadet/' + pk + '/eliminar/',
                    {
                        method: 'DELETE',
                        headers: headers // Agregar el encabezado X-CSRFToken a la solicitud DELETE
                    })
...
if (document.getElementById('labtitdetmod').textContent==="INGRESAR-DETALLE") {ordenventaradet_guardar(); }
if (document.getElementById('labtitdetmod').textContent==="EDITAR-DETALLE") {ordenventaradet_editar(); }
var ob="#btnguardardet";  $(''+ ob +'').prop('disabled', false);
}


function ordenventaradet_editar(){
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/ordenventadet_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pkov': pkov,  'orden':orden,'codigo':codigo,'cantidad':cantidad,'precio':precio },
    success: function(data) {
 if (data.success) {
...
function ordenventaradet_guardar(){
var pkov = document.getElementById('id_pkov').value;
var codigo = document.getElementById('codigo').value;
var cantidad = document.getElementById('cantidad').value;
var precio = document.getElementById('precio').value;
...
$.ajax({
     url: "/ordenventadet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pkov': pkov, 'codigo':codigo,'cantidad':cantidad,'precio':precio },
     success: function(data) {
 if (data.success) {
        Swal.fire({  title: '',  text: 'ACTUALIZADO',
        icon: 'success',  customClass: {container: 'msnsuccess', }});
...
function editarordenventaadet(orden){
