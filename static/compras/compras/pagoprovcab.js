...
var url = '/pagoprovcab_filtro_lst/?' + params;
...
function pagoprovfact(){
var id_pkf = document.getElementById('id_pkf').value;
if (id_pkf.length>10){
orden=0;
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/pagoprovfact_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
...
for (var i = 0; i < list.length; i++) {
      ...
      html += '<div class ="campbtn" > <button  class="btnlstm" onClick="editarcompradet('+ pagofact.orden +')" >  EDITAR </button> ';
      html += '	<a class="btnlste" href="/pagoprovfact/'+pagofact.pkfd+'/eliminar/" >ELIMINAR</a>';
...
}else{html=listadocab(); $('#pagoprovfact-list').html(html);  }
}
...
function listadoagregardetalle(lin,e){
      ...
      html += '<button  id="btncancelardetfact" class="btnlstm" onClick="cancdetfact()">CANCELAR </button>  ';
      html += '</div> ';
      html += '</div> ';
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/pagoprovfact_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
...
function pagoprovpago(){
var id_pkf = document.getElementById('id_pkf').value;
if (id_pkf.length>10){
orden=0;
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/pagoprovpago_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'id_pk': id_pkf,'orden': orden},
     success: function(data) {
    if (data.success) {
     var list = data.datos;
...
for (var i = 0; i < list.length; i++) {
      ...
      html += '	<a class="btnlste" href="/pagoprovpago/'+pago.pkpf+'/eliminar/" >ELIMINAR</a>';
      html += '</div> ';
      html += '</div> ';
      html += '</div> ';
...
}else{ html=listadocabp();  $('#pagoprovfpago-list').html(html);}
}
...
function listadoagregardetallep(lin,e){
      ...
      html += '<div class ="campbtn" > <button  id="btnguardardetp" class="btnlstm" onClick="guardardetp()" >  GUARDAR </button>  ';
      html += ' <button  id="btnguardardetp4" class="btnlstm" onClick="limpiardetp(2)" >  CANCELAR </button>  ';
      html += '</div> ';
      html += '</div> ';
      html += '</div> ';// id= "pagofactmod"';
    return html;
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/pagoprovpago_agregar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
