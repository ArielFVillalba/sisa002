...
function filtrarnotadebitodecompra(){
...
var url = '/notadebitocompcab_filtro_lst/?' + params;
...
function buscarnotadebitodecompra(){
...
var url = "/notadebitocompcab/" + variable + "/listar/";  // Construir la URL de la vista
...
function eliminar_notadebitondecompracab(pk) {
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/notadebitocompcab/' + pk + '/eliminar/', { method: 'DELETE', headers: headers  })
                    .then((response) => {
                    if (response.ok) {
                        ...
                        Swal.fire({  title: '',  text: 'eliminado exitosamente ',
                         ...
                         document.location.href= "/notadebitocompcab/crear/";
...
if (document.querySelector('#notadebitocomp_frm')) {
busdetalle();
cargarcmbart();
}
}
...
var id_pknd = document.getElementById('id_pknd').value;
if (id_pknd.length>10){

    orden=0;
var csrftoken = $("[name=csrfmiddlewaretoken]").val();

$.ajax({
     url: "/notadebitocompdet_listar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pknd': id_pknd,'orden': orden},
...
for (var i = 0; i < list.length; i++) {
      ...
      html += '<div class ="campcantiva"  > <input   id="orden'+ e +'"  value="'+ notadebitocompdet.orden + '"</input> </div>';
      html += '<div class ="camplst" > <input id="codigo'+ e +'" value="'+ notadebitocompdet.codigo + '"</input> </div>';
      html += '<div class ="campdescr" > <input id="descripcion'+ e +'"  list="descrip_list"  value=" '+ notadebitocompdet.descripcion + '"</input> </div>';
      html += '</div> ';
      html += '<div id= "lin'+ lin +'" class ="detallelinea2"> ';
      html += '<div class ="campcantiva" > <input id="cantidad'+ e +'" value=" '+ number_format(notadebitocompdet.cantidad,3) + '"</input> </div>';
      html += '<div class ="camplst" > <input id="precio'+ e +'" value=" '+ number_format(notadebitocompdet.precio,0) + '"</input> </div>';
      html += '<div class ="campcantiva" > <input id="iva'+ e +'" value=" '+ notadebitocompdet.iva + '"</input> </div>';
      html += '<div class ="camplst" > <input id="subtotal'+ e +'" value=" '+number_format(notadebitocompdet.subtotal,0) + '"</input> </div>';
      html += '<div class ="campbtn" > <button  class="btnlstdet" onClick="editarnotadebitocompdet('+ notadebitocompdet.orden +')" >  EDITAR </button> ';
      html += '<button  class="btnlstdet" onClick="notadebitocompdet_eliminar(\''+notadebitocompdet.pkndd+'\' )" >ELIMINAR </button> </div>';
      html += '</div> ';
      html += '</div> ';
      total=total+(notadebitocompdet.cantidad*notadebitocompdet.precio);
...
function notadebitocompdet_eliminar(pk){
 // Obtener el token CSRF de la cookie
  ...
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/notadebitocompdet/' + pk + '/eliminar/',
                    {
                        method: 'DELETE',
...
function notadebitocompdet_eliminareed(pk){
 // Obtener el token CSRF de la cookie
  const csrftoken = document.cookie.match(/csrftoken=([\w-]+)/)[1];
  // Configurar el encabezado X-CSRFToken en la solicitud DELETE
  const headers = {'X-CSRFToken': csrftoken};
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    fetch('/notacreditocompdet/' + pk + '/eliminar/',
                    {
                        method: 'DELETE',
                        headers: headers // Agregar el encabezado X-CSRFToken a la solicitud DELETE
                    })
...
function notadebitocompdet_eliminaree(pk){
 // Obtener el token CSRF de la cookie
  const csrftoken = document.cookie.match(/csrftoken=([\w-]+)/)[1];
  // Configurar el encabezado X-CSRFToken en la solicitud DELETE
  const headers = {'X-CSRFToken': csrftoken};
...
function editarnotadebitocompdet(orden){
...
if (document.getElementById('labtitdetmod').textContent==="INGRESAR-DETALLE") {notadebitocompradet_guardar(); }
if (document.getElementById('labtitdetmod').textContent==="EDITAR-DETALLE") {notadebitocompradet_editar(); }
var ob="#btnguardardet";  $(''+ ob +'').prop('disabled', false);

}


function notadebitocompradet_editar(){
var orden = document.getElementById('orden').value;
var pknd = document.getElementById('id_pknd').value;
...
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/notadebitocompdet_editar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pknd': pknd,  'orden':orden,'codigo':codigo,'cantidad':cantidad,'precio':precio },
    success: function(data) {
    if (data.success) {
...
function notadebitocompradet_guardar(){
var pknd = document.getElementById('id_pknd').value;
var codigo = document.getElementById('codigo').value;
var cantidad = document.getElementById('cantidad').value;
var precio = document.getElementById('precio').value;
//alert ("  pknd  " + pknd+ " " +"  codigo  " + codigo + " " +"  cantidad  " + cantidad + " "  +"  precio  " + precio  )
valida=true;
if (valida==true){
//usuario = obtener_cookie_por_nombre("usuario");
//contraseña = obtener_cookie_por_nombre("contraseña");
var csrftoken = $("[name=csrfmiddlewaretoken]").val();
$.ajax({
     url: "/notadebitocompdet_guardar/",  type: 'POST', datatype: 'json',  headers:{ "X-CSRFToken": csrftoken    },
     data: { 'pknd': pknd, 'codigo':codigo,'cantidad':cantidad,'precio':precio },
     success: function(data) {
