...
def notacreditodeventacabinf_listar(request):
    ...
    notacreditodeventacab = Notacreditoventacab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idcliente == '':
        idcliente = 0
    if notacreditodeventacab.exists() and int(idcliente) >= 1:
        notacreditodeventacab = notacreditodeventacab .filter(idcliente_id=idcliente,idempresa=didempresa,idsucursal=didsucursal)
    if notacreditodeventacab.exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        notacreditodeventacab= notacreditodeventacab .filter(fecha__gte=fecha_inicio,idempresa=didempresa,idsucursal=didsucursal)
    if notacreditodeventacab.exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        notacreditodeventacab= notacreditodeventacab .filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    if notacreditodeventacab.exists() and tipodoc:
        notacreditodeventacab= notacreditodeventacab .filter(tipodoc=tipodoc,idempresa=didempresa,idsucursal=didsucursal)

    datos = []
    for objeto in notacreditodeventacab :
        datos.append({
    ...
    total = sum([objeto.total * objeto.total for objeto in notacreditodeventacab ])
    Response = JsonResponse({'datos': datos})
    return Response


def notacreditodeventacab_inf(request):
    if not request.user.is_authenticated:
        return redirect('login')

    return render(request, 'ventasinf/lstnotacreditoventa.html')

def notacreditodeventacabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    # -----------------------------------------------
    ...
    objetos=notacreditodeventacab 

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "NOTA CREDITO ", "",""])

    writer.writerow(["FECHA", "NRO NOTA CREDITO", "CLIENTE", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nronota, fila.cliente, fila.tipodoc, fila.total])
    return response

def notacreditodeventadetinfcsv(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "INFORME DE NOTA CREDITO DE VENTA DETALLADO ","","","" ,"","","",""])

    writer.writerow(["FECHA", "FACTURA", "CLIENTE","ORDEN","codigo","descripcion" ,"cantidad","precio","iva","subtotal"])
    total=0
    for objetocab in notacreditodeventacab :
        didnotacreditoventacab = objetocab.idnotacreditoventacab
        notacreditodeventadet = Notacreditoventadet.objects.filter(idnotacreditoventacab =didnotacreditoventacab).order_by('orden')

        for objetodet in notacreditodeventadet:
...
def notacreditodeventacabinfexcel(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    headers=["FECHA", "NRO NOTA CREDITO", "CLIENTE", "TIPODOC","TOTAL"]
    ...
    titulo_general = "LISTADO NOTA CREDIO VENTA"
...
def notacreditodeventadetinfexcel(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    inicio_celda.alignment = Alignment(horizontal='center')
    # Establece el valor del título general en la celda fusionada
    titulo_general = "LISTADO NOTA CREDITO DE  VENTAS DETALLADO  "
    inicio_celda.value = titulo_general
    row_num=3
    total=0
    for objetocab in notacreditodeventacab :
...
def notacreditodeventacabinfpdf(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    if notacreditodeventacab.exists() and idcliente != '' :
        notacreditodeventacab = notacreditodeventacab .filter(idcliente_id=idcliente,idempresa=didempresa,idsucursal=didsucursal)
    if notacreditodeventacab.exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        notacreditodeventacab = notacreditodeventacab .filter(fecha__gte=fecha_inicio,idempresa=didempresa,idsucursal=didsucursal)
    ...
    notacreditodeventacab_infpdftit(c,680,0)
    notacreditodeventacab_infpdfdet(notacreditodeventacab ,c,620)
    c.save()
    buf.seek(0)
...
def notacreditodeventacab_infpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME NOTA CREDITO DE VENTA ")
        notacreditodeventacab_infpdfcabdet(c,620)


def notacreditodeventacab_infpdfcabdet(c,y):
    data = [["FECHA", "N ORDEN", "CLIENTE", "TIPODOC","TOTAL"]]

    # Definir conotacreditoadas y dimensiones de la tabla
    fila_x = 20  # Conotacreditoada x de la esquina superior izquierda de la tabla
    fila_y = y  # Conotacreditoada y de la esquina superior izquierda de la tabla
...
def notacreditodeventacab_infpdfdet(notacreditodeventacab ,c,y):
        total=0
        detmax=25
        nropag=1
        ...
        # Definir conotacreditoadas y dimensiones de la tabla
        fila_x = 30  # Conotacreditoada x de la esquina superior izquierda de la tabla
        fila_y = y  # Conotacreditoada y de la esquina superior izquierda de la tabla
        ancho_col = [80, 80, 100, 60, 100]  # Ancho de cada columna de la tabla 100 3.5 cm 20 7 cm
        alto_fila = 20  # Alto de cada fila de la tabla
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                notacreditodeventacab_infpdftit(c, 680, nropag)
                fila_y=620

            # Calcular conotacreditoadas de la fila actual
...
def notacreditodeventadetinf_listar(request):
    idcliente = request.POST['idcliente']
    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    tipodoc = request.POST['tipodoc']
...
def notacreditodeventadetinfpdf(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    notacreditodeventadetinfpdftit(c,680,0)
    notacreditodeventadetinfpdfdet(notacreditodeventacab ,c,620)
    c.save()
    buf.seek(0)
...
def notacreditodeventadetinfpdftit(c,y,npag):
        ahora = time.strftime("%c")
        fechahora = time.strftime("%c")
        fecha = time.strftime("%x")
        print(npag)
        ...
        c.setFillColor(colors.black)
        c.setFont("Helvetica", 22)  # Puedes ajustar la fuente y el tamaño según tus necesidades
        c.drawString(lx -10, hi-20,"INF. NOTA CREDITO  DE VENTA DETALLE ")
        notacreditodeventadetinfcabdet(c,620)


def notacreditodeventadetinfcabdet(c,y):
...
def notacreditodeventadetinfpdfdet(notacreditodeventacab ,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir conotacreditoadas y dimensiones de la tabla
        ...
        for objetocab in notacreditodeventacab :
            didnotacreditoventacab = objetocab.idnotacreditoventacab
            notacreditodeventadet = Notacreditoventadet.objects.filter(idnotacreditoventacab =didnotacreditoventacab).order_by('orden')
            if notacreditodeventadet.exists():
                fila_x=30
                fila_y = fila_y - alto_fila
                ...
                data = []
                for objetodet in notacreditodeventadet:
                    notacreditomax=objetodet.orden
                    nueva_fila = [
                      str(objetodet.orden),
                ...
                for i, fila in enumerate(data):
                    ...
                    if e > detmax:
                        ...
                        nropag = nropag+1
                        e=0
                        notacreditodeventadetinfpdftit(c, 680, nropag)
                        fila_y=620
