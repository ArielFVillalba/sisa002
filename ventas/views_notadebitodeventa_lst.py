...
def notadebitodeventacabinf_listar(request):
    ...
    notadebitodeventacab = Notadebitoventacab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idcliente == '':
        idcliente = 0
    if notadebitodeventacab.exists() and int(idcliente) >= 1:
        notadebitodeventacab = notadebitodeventacab .filter(idcliente_id=idcliente)
    if notadebitodeventacab.exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        notadebitodeventacab= notadebitodeventacab .filter(fecha__gte=fecha_inicio)
    if notadebitodeventacab.exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        notadebitodeventacab= notadebitodeventacab .filter(fecha__lte=fecha_fin)
    if notadebitodeventacab.exists() and tipodoc:
        notadebitodeventacab= notadebitodeventacab .filter(tipodoc=tipodoc)

    datos = []
    for objeto in notadebitodeventacab :
        datos.append({
        'fecha': objeto.fecha.strftime("%d/%m/%Y"),
        'nrodebito': objeto.nrodebito,
        'cliente': objeto.cliente,
        'tipodoc': objeto.tipodoc,
        'total':objeto.total
    })
    total = sum([objeto.total * objeto.total for objeto in notadebitodeventacab ])
    Response = JsonResponse({'datos': datos})
    return Response


def notadebitodeventacab_inf(request):
    if not request.user.is_authenticated:
        return redirect('login')

    return render(request, 'ventasinf/lstnotadebitoventa.html')

def notadebitodeventacabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    objetos=notadebitodeventacab 

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "NOTA DEBITO DE VENTA", "",""])

    writer.writerow(["FECHA", "NRO NOTA DEBITO", "CLIENTE", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nrodebito, fila.cliente, fila.tipodoc, fila.total])
    return response

def notadebitodeventadetinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "INFORME DE NOTA DEBITO DE VENTA DETALLADO ","","","" ,"","","",""])

    writer.writerow(["FECHA", "FACTURA", "CLIENTE","ORDEN","codigo","descripcion" ,"cantidad","precio","iva","subtotal"])
    total=0
    for objetocab in notadebitodeventacab :
        didnotadebitoventacab = objetocab.idnotadebitoventacab
        notadebitodeventadet = Notadebitoventadet.objects.filter(idnotadebitoventacab =didnotadebitoventacab).order_by('orden')

        for objetodet in notadebitodeventadet:
...
def notadebitodeventacabinfexcel(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    headers=["FECHA", "NRO NOTA DEBITO", "CLIENTE", "TIPODOC","TOTAL"]
    ...
    for row_num, fila in enumerate(data, 3):
        ...
        sheet.cell(row=row_num, column=2, value=fila.nrodebito)
...
def notadebitodeventadetinfexcel(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    inicio_celda.alignment = Alignment(horizontal='center')
    # Establece el valor del título general en la celda fusionada
    titulo_general = "LISTADO NOTA DEBITO DE  VENTAS DETALLADO  "
    inicio_celda.value = titulo_general
    row_num=3
    total=0
    for objetocab in notadebitodeventacab :
        ...
        for objetodet in notadebitodeventadet:
            ...
            sheet.cell(row=row_num, column=2, value=objetocab.nrodebito)
...
def notadebitodeventacabinfpdf(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    notadebitodeventacab_infpdftit(c,680,0)
    notadebitodeventacab_infpdfdet(notadebitodeventacab ,c,620)
    c.save()
    buf.seek(0)
...
def notadebitodeventacab_infpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME NOTA DEBITO DE VENTA ")
        notadebitodeventacab_infpdfcabdet(c,620)


def notadebitodeventacab_infpdfcabdet(c,y):
    data = [["FECHA", "N ORDEN", "CLIENTE", "TIPODOC","TOTAL"]]

    # Definir conotadebitoadas y dimensiones de la tabla
    fila_x = 20  # Conotadebitoada x de la esquina superior izquierda de la tabla
    fila_y = y  # Conotadebitoada y de la esquina superior izquierda de la tabla
...
def notadebitodeventacab_infpdfdet(notadebitodeventacab ,c,y):
        total=0
        detmax=25
        nropag=1
        ...
        # Definir conotadebitoadas y dimensiones de la tabla
        fila_x = 30  # Conotadebitoada x de la esquina superior izquierda de la tabla
        fila_y = y  # Conotadebitoada y de la esquina superior izquierda de la tabla
        ancho_col = [80, 80, 100, 60, 100]  # Ancho de cada columna de la tabla 100 3.5 cm 20 7 cm
        alto_fila = 20  # Alto de cada fila de la tabla
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                notadebitodeventacab_infpdftit(c, 680, nropag)
                fila_y=620

            # Calcular conotadebitoadas de la fila actual
...
def notadebitodeventadetinf_listar(request):
    idcliente = request.POST['idcliente']
    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    tipodoc = request.POST['tipodoc']
...
def notadebitodeventadetinfpdf(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    notadebitodeventacab = Notadebitoventacab.objects.all(idempresa=didempresa,idsucursal=didsucursal)
    if idcliente == '':
        idcliente = 0
    if notadebitodeventacab.exists() and int(idcliente) >= 1:
        notadebitodeventacab = notadebitodeventacab.filter(idcliente_id=idcliente)
    ...
    notadebitodeventadetinfpdftit(c,680,0)
    notadebitodeventadetinfpdfdet(notadebitodeventacab ,c,620)
    c.save()
    buf.seek(0)
...
def notadebitodeventadetinfpdftit(c,y,npag):
        ahora = time.strftime("%c")
        fechahora = time.strftime("%c")
        fecha = time.strftime("%x")
        print(npag)
        ...
        c.setFillColor(colors.black)
        c.setFont("Helvetica", 22)  # Puedes ajustar la fuente y el tamaño según tus necesidades
        c.drawString(lx -10, hi-20,"INF. NOTA DEBITO  DE VENTA DETALLE ")
        notadebitodeventadetinfcabdet(c,620)


def notadebitodeventadetinfcabdet(c,y):
...
def notadebitodeventadetinfpdfdet(notadebitodeventacab ,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir conotadebitoadas y dimensiones de la tabla
        ...
        for objetocab in notadebitodeventacab :
            didnotadebitoventacab = objetocab.idnotadebitoventacab
            notadebitodeventadet = Notadebitoventadet.objects.filter(idnotadebitoventacab =didnotadebitoventacab).order_by('orden')
            if notadebitodeventadet.exists():
                fila_x=30
                fila_y = fila_y - alto_fila
                ...
                fila_x=fila_x+50
                celda(c, objetocab.nrodebito, fila_x, fila_y, 10, alto_fila,"t", "l", 20, 0)
                fila_x=fila_x+60
                ...
                for i, fila in enumerate(data):
                    ...
                    if e > detmax:
                        ...
                        nropag = nropag+1
                        e=0
                        notadebitodeventadetinfpdftit(c, 680, nropag)
                        fila_y=620
