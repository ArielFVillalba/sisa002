...
def pedidodeventacabinf_listar(request):
    ...
    if int(didempresa) < 1 or int(didsucursal) < 1:
        return redirect('login')
    pedidodeventacab = Pedidoventacab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idcliente == '':
        idcliente = 0
    if pedidodeventacab.exists() and int(idcliente) >= 1:
        pedidodeventacab   = pedidodeventacab .filter(idcliente_id=idcliente)
    if pedidodeventacab .exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        pedidodeventacab   = pedidodeventacab .filter(fecha__gte=fecha_inicio)
    if pedidodeventacab .exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        pedidodeventacab   = pedidodeventacab .filter(fecha__lte=fecha_fin)
    if pedidodeventacab .exists() and tipodoc:
        pedidodeventacab   = pedidodeventacab .filter(tipodoc=tipodoc)

    datos = []
    for objeto in pedidodeventacab :
        datos.append({
    ...
    total = sum([objeto.total * objeto.total for objeto in pedidodeventacab ])
    Response = JsonResponse({'datos': datos})
    return Response


def pedidodeventacab_inf(request):
    if not request.user.is_authenticated:
        return redirect('login')

    return render(request, 'ventasinf/lstpedidodeventa.html')

def pedidodeventacabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    objetos=pedidodeventacab 

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "PEDIDO DE VENTA", "",""])

    writer.writerow(["FECHA", "NRO PEDIDO", "CLIENTE", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nropedido, fila.cliente, fila.tipodoc, fila.total])
    return response

def pedidodeventadetinfcsv(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "INFORME DE PEDIDO DE VENTA DETALLADO ","","","" ,"","","",""])

    writer.writerow(["FECHA", "FACTURA", "CLIENTE","ORDEN","codigo","descripcion" ,"cantidad","precio","iva","subtotal"])
    total=0
    for objetocab in pedidodeventacab :
        didpedidoventacab = objetocab.idpedidoventacab
        pedidodeventadet = Pedidoventadet.objects.filter(idpedidoventacab =didpedidoventacab).order_by('orden')

        for objetodet in pedidodeventadet:
...
def pedidodeventacabinfexcel(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    headers=["FECHA", "NRO PEDIDO", "CLIENTE", "TIPODOC","TOTAL"]
...
def pedidodeventadetinfexcel(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    inicio_celda.alignment = Alignment(horizontal='center')
    # Establece el valor del título general en la celda fusionada
    titulo_general = "LISTADO PEDIDO DE  VENTAS DETALLADO  "
    inicio_celda.value = titulo_general
    row_num=3
    total=0
    for objetocab in pedidodeventacab :
...
def pedidodeventacabinfpdf(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    pedidodeventacab_infpdftit(c,680,0)
    pedidodeventacab_infpdfdet(pedidodeventacab ,c,620)
    c.save()
    buf.seek(0)
...
def pedidodeventacab_infpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME PEDIDO DE VENTA ")
        pedidodeventacab_infpdfcabdet(c,620)


def pedidodeventacab_infpdfcabdet(c,y):
    data = [["FECHA", "N ORDEN", "CLIENTE", "TIPODOC","TOTAL"]]

    # Definir copedidoadas y dimensiones de la tabla
    fila_x = 20  # Copedidoada x de la esquina superior izquierda de la tabla
    fila_y = y  # Copedidoada y de la esquina superior izquierda de la tabla
...
def pedidodeventacab_infpdfdet(pedidodeventacab ,c,y):
        total=0
        detmax=25
        nropag=1
        ...
        # Definir copedidoadas y dimensiones de la tabla
        fila_x = 30  # Copedidoada x de la esquina superior izquierda de la tabla
        fila_y = y  # Copedidoada y de la esquina superior izquierda de la tabla
        ancho_col = [80, 80, 100, 60, 100]  # Ancho de cada columna de la tabla 100 3.5 cm 20 7 cm
        alto_fila = 20  # Alto de cada fila de la tabla
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                pedidodeventacab_infpdftit(c, 680, nropag)
                fila_y=620

            # Calcular copedidoadas de la fila actual
...
def pedidodeventadetinf_listar(request):
    idcliente = request.POST['idcliente']
    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    tipodoc = request.POST['tipodoc']
    ...
    if int(didempresa) < 1 or int(didsucursal) < 1:
        return redirect('login')
    pedidodeventacab   = Pedidoventacab.objects.filter(idempres=idempresa,idsucursal=idsucursal)
    if idcliente == '':
        idcliente = 0
    if pedidodeventacab .exists() and int(idcliente) >= 1:
        pedidodeventacab   = pedidodeventacab .filter(idcliente_id=idcliente)
...
def pedidodeventadetinfpdf(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    pedidodeventadetinfpdftit(c,680,0)
    pedidodeventadetinfpdfdet(pedidodeventacab ,c,620)
    c.save()
    buf.seek(0)
...
def pedidodeventadetinfpdftit(c,y,npag):
        ahora = time.strftime("%c")
        fechahora = time.strftime("%c")
        fecha = time.strftime("%x")
        print(npag)
        ...
        c.setFillColor(colors.black)
        c.setFont("Helvetica", 22)  # Puedes ajustar la fuente y el tamaño según tus necesidades
        c.drawString(lx -10, hi-20,"INF. PEDIDO  DE VENTA DETALLE ")
        pedidodeventadetinfcabdet(c,620)


def pedidodeventadetinfcabdet(c,y):
...
def pedidodeventadetinfpdfdet(pedidodeventacab ,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir copedidoadas y dimensiones de la tabla
            ...
            if pedidodeventadet.exists():
                ...
                for i, fila in enumerate(data):
                    ...
                    if e > detmax:
                        ...
                        nropag = nropag+1
                        e=0
                        pedidodeventadetinfpdftit(c, 680, nropag)
                        fila_y=620
