...
def ventacabinf_listar(request):
    ...
    total = sum([objeto.total * objeto.total for objeto in ventacab])
    Response = JsonResponse({'datos': datos})
    return Response


def ventacabinf(request):
    if not request.user.is_authenticated:
        return redirect('login')

    return render(request, 'ventasinf/lstventas.html')

def ventacabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    print(idcliente + fechaini + fechafin + tipodoc)
    ventacab = Ventacab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idcliente == '':
        idcliente = 0
    if ventacab.exists() and int(idcliente) >= 1:
    ...
    writer.writerow(["", "", "LSITADO DE VENTA", ""])

    writer.writerow(["FECHA", "FACTURA", "CLIENTE", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nrofactura, fila.cliente, fila.tipodoc, fila.total])
    return response


def ventadetinfcsv(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "LSITADO DE VENTA DETALLADO", ""])

    writer.writerow(["FECHA", "FACTURA", "CLIENTE","ORDEN","codigo","descripcion" ,"cantidad","precio","iva","subtotal"])
    total=0
    for objetocab in ventacab:
        idiventa = objetocab.idventacab
        ventadet = Ventadet.objects.filter(idventacab=idiventa).order_by('orden')

        for objetodet in ventadet:
...
def ventacabinfexcel(request):
    ...
    sheet = wb.active
    sheet.title = "LISTADO VENTA"

    # Agrega encabezados a la hoja de cálculo
    # headers = ['Campo1', 'Campo2', 'Campo3']
    headers=["FECHA", "FACTURA", "CLIENTE", "TIPODOC","TOTAL"]
...
def ventadetinfexcel(request):
    ...
    inicio_celda.alignment = Alignment(horizontal='center')
    # Establece el valor del título general en la celda fusionada
    titulo_general = "LISTADO VENTA DETALLADO  "
    inicio_celda.value = titulo_general
    row_num=3
    total=0
    for objetocab in ventacab:
...
def ventacabinfpdf(request):
    ...
    ventacabinfpdftit(c,680,0)
    ventacabinfpdfdet(ventacab,c,620)
    c.save()
    buf.seek(0)

    return FileResponse(buf, filename='venta.pdf')
...
def ventacabinfpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME VENTA ")
        ventacabinfpdfcabdet(c,620)


def ventacabinfpdfcabdet(c,y):
...
def ventacabinfpdfdet(ventacab,c,y):
        total=0
        detmax=25
        nropag=1
        #data = [["FECHA", "FACTURA", "CLIENTE", "TIPODOC","TOTAL"]]
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                ventacabinfpdftit(c, 680, nropag)
                fila_y=620
...
def ventadetinf_listar(request):
    idcliente = request.POST['idcliente']
    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    tipodoc = request.POST['tipodoc']
...
def ventadetinfpdf(request):
    ...
    ventadetinfpdftit(c,680,0)
    ventadetinfpdfdet(ventacab,c,620)
    c.save()
    buf.seek(0)
...
def ventadetinfpdftit(c,y,npag):
        ahora = time.strftime("%c")
        fechahora = time.strftime("%c")
        fecha = time.strftime("%x")
        print(npag)
        ...
        c.setFillColor(colors.black)
        c.setFont("Helvetica", 22)  # Puedes ajustar la fuente y el tamaño según tus necesidades
        c.drawString(lx + 20, hi-20,"INFORME VENTA DETALLE ")
        ventadetinfcabdet(c,620)


def ventadetinfcabdet(c,y):
...
def ventadetinfpdfdet(ventacab,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir coordenadas y dimensiones de la tabla
            ...
            if ventadet.exists():
                ...
                for i, fila in enumerate(data):
                    ...
                    if e > detmax:
                        ...
                        nropag = nropag+1
                        e=0
                        ventadetinfpdftit(c, 680, nropag)
                        fila_y=620
