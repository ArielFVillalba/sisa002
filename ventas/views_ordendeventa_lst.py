...
def ordendeventacabinf_listar(request):
    ...
    ordendeventacab = Ordenventacab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idcliente == '':
        idcliente = 0
    if ordendeventacab.exists() and int(idcliente) >= 1:
        ordendeventacab   = ordendeventacab .filter(idcliente_id=idcliente)
    if ordendeventacab .exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        ordendeventacab   = ordendeventacab .filter(fecha__gte=fecha_inicio)
    if ordendeventacab .exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        ordendeventacab   = ordendeventacab .filter(fecha__lte=fecha_fin)
    if ordendeventacab .exists() and tipodoc:
        ordendeventacab   = ordendeventacab .filter(tipodoc=tipodoc)

    datos = []
    for objeto in ordendeventacab :
        datos.append({
    ...
    total = sum([objeto.total * objeto.total for objeto in ordendeventacab ])
    Response = JsonResponse({'datos': datos})
    return Response


def ordendeventacab_inf(request):
    if not request.user.is_authenticated:
        return redirect('login')

    return render(request, 'ventasinf/lstordendeventa.html')

def ordendeventacabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    objetos=ordendeventacab 

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "ORDEN DE VENTA", "",""])

    writer.writerow(["FECHA", "NRO ORDEN", "CLIENTE", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nroorden, fila.cliente, fila.tipodoc, fila.total])
    return response

def ordendeventadetinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "INFORME DE ORDEN DE VENTA DETALLADO ","","","" ,"","","",""])

    writer.writerow(["FECHA", "FACTURA", "CLIENTE","ORDEN","codigo","descripcion" ,"cantidad","precio","iva","subtotal"])
    total=0
    for objetocab in ordendeventacab :
        didordenventacab = objetocab.idordenventacab
        ordendeventadet = Ordenventadet.objects.filter(idordenventacab =didordenventacab).order_by('orden')

        for objetodet in ordendeventadet:
...
def ordendeventacabinfexcel(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    sheet = wb.active
    sheet.title = "Listado ORDEN DE VENTAS"

    # Agrega encabezados a la hoja de cálculo
    # headers = ['Campo1', 'Campo2', 'Campo3']
    headers=["FECHA", "NRO ORDEN", "CLIENTE", "TIPODOC","TOTAL"]
    ...
    titulo_general = "LISTADO ORDEN DE VENTAS"
...
def ordendeventadetinfexcel(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    # Crea una hoja de cálculo en el libro de trabajo
    sheet = wb.active
    sheet.title = "LISTADO ORDEN DE  VENTAS DETALLADO"
    ...
    inicio_celda.alignment = Alignment(horizontal='center')
    # Establece el valor del título general en la celda fusionada
    titulo_general = "LISTADO VENTAS DETALLADO  "
    inicio_celda.value = titulo_general
    row_num=3
    total=0
    for objetocab in ordendeventacab :
...
def ordendeventacabinfpdf(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    ordendeventacab_infpdftit(c,680,0)
    ordendeventacab_infpdfdet(ordendeventacab ,c,620)
    c.save()
    buf.seek(0)
...
def ordendeventacab_infpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME ORDEN DE VENTA ")
        ordendeventacab_infpdfcabdet(c,620)


def ordendeventacab_infpdfcabdet(c,y):
...
def ordendeventacab_infpdfdet(ordendeventacab ,c,y):
        total=0
        detmax=25
        nropag=1
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                ordendeventacab_infpdftit(c, 680, nropag)
                fila_y=620
...
def ordendeventadetinf_listar(request):
    idcliente = request.POST['idcliente']
    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    tipodoc = request.POST['tipodoc']
    ...
    datos = []
    for objetocab in ordendeventacab:
        idiventa =objetocab.idordenventacab
        ordendeventadet = Ordenventadet.objects.filter(idordenventacab =idiventa).order_by('orden')
        for objetodet in ordendeventadet:
             datos.append({
...
def ordendeventadetinfpdf(request):
    # -----------------------------------------------
    idcliente = request.GET.get('idcliente', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    ordedeventadetinfpdftit(c,680,0)
    ordedeventadetinfpdfdet(ordendeventacab ,c,620)
    c.save()
    buf.seek(0)
...
def ordedeventadetinfpdftit(c,y,npag):
        ahora = time.strftime("%c")
        fechahora = time.strftime("%c")
        fecha = time.strftime("%x")
        print(npag)
        ...
        c.setFillColor(colors.black)
        c.setFont("Helvetica", 22)  # Puedes ajustar la fuente y el tamaño según tus necesidades
        c.drawString(lx -10, hi-20,"INF. ORDEN  DE VENTA DETALLE ")
        ordedeventadetinfcabdet(c,620)


def ordedeventadetinfcabdet(c,y):
...
def ordedeventadetinfpdfdet(ordendeventacab ,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir coordenadas y dimensiones de la tabla
            ...
            if ordendeventadet.exists():
                ...
                for i, fila in enumerate(data):
                    ...
                    if e > detmax:
                        ...
                        nropag = nropag+1
                        e=0
                        ordedeventadetinfpdftit(c, 680, nropag)
                        fila_y=620
