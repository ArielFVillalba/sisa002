...
def pedidodecompracabinf_listar(request):
    ...
    pedidodecompracab = Pedidocompcab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idproveedor == '':
        idproveedor = 0
    if pedidodecompracab.exists() and int(idproveedor) >= 1:
        pedidodecompracab   = pedidodecompracab .filter(idproveedor_id=idproveedor,idempresa=didempresa,idsucursal=didsucursal)
    if pedidodecompracab .exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        pedidodecompracab   = pedidodecompracab .filter(fecha__gte=fecha_inicio,idempresa=didempresa,idsucursal=didsucursal)
    if pedidodecompracab .exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        pedidodecompracab   = pedidodecompracab .filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    if pedidodecompracab .exists() and tipodoc:
        pedidodecompracab   = pedidodecompracab .filter(tipodoc=tipodoc,idempresa=didempresa,idsucursal=didsucursal)

    datos = []
    for objeto in pedidodecompracab:
        datos.append({
    ...
    total = sum([objeto.total * objeto.total for objeto in pedidodecompracab ])
    Response = JsonResponse({'datos': datos})
    return Response


def pedidodecompracab_inf(request):
    return render(request, 'comprasinf/lstpedidocompra.html')

def pedidpdecompracabinfcsv(request):
    # -----------------------------------------------
    ...
    objetos=pedidodecompracab 


    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "LISTADO PEDIDO DE COMPRA", "",""])

    writer.writerow(["FECHA", "NRO PEDIDO","PROVEEDOR", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nropedido, fila.proveedor, fila.tipodoc, fila.total])
    return response

def pedidodecompradetinfcsv(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "INFORME DE PEDIDO DE COMPRA","","","" ,"","","",""])

    writer.writerow(["FECHA", "NRO PEDIDO", "PROVEEDOR","ORDEN","CODIGO","DESCRIPCION" ,"CANTIDAD","PRECIO","IVA","SUBTOTAL"])
    total=0
    for objetocab in pedidodecompracab :
        didpedidocompcab = objetocab.idpedidocompcab
        pedidodecompradet = Pedidocompdet.objects.filter(idpedidocompcab =didpedidocompcab).order_by('orden')

        for objetodet in pedidodecompradet:
...
def pedidodecompracabinfexcel(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    pedidodecompracab   = Pedidocompcab.objects.all()
    if idproveedor == '':
        idproveedor = 0
    if pedidodecompracab .exists() and int(idproveedor) >= 1:
        pedidodecompracab   = pedidodecompracab .filter(idproveedor_id=idproveedor)
    if pedidodecompracab .exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        pedidodecompracab   = pedidodecompracab .filter(fecha__gte=fecha_inicio)
    if pedidodecompracab .exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        pedidodecompracab   = pedidodecompracab .filter(fecha__lte=fecha_fin)
    if pedidodecompracab .exists() and tipodoc:
        pedidodecompracab   = pedidodecompracab .filter(tipodoc=tipodoc)

    # -------------------------------------------------
    data = pedidodecompracab 
    ...
    sheet = wb.active
    sheet.title = "Listado  PEDIDO DE COMPRAS"
    ...
    titulo_general = "LISTADO PEDIDO DE COMPRAS"
    ...
    for row_num, fila in enumerate(data, 3):
        ...
        sheet.cell(row=row_num, column=2, value=fila.nropedido)
...
def pedidodecompradetinfexcel(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    # Crea una hoja de cálculo en el libro de trabajo
    sheet = wb.active
    sheet.title = "LISTADO ORDEN DE  COMPRAS"
    ...
    inicio_celda.alignment = Alignment(horizontal='center')
    # Establece el valor del título general en la celda fusionada
    titulo_general = "LISTADO PEDIDO DE COMPRAS DETALLADO  "
    inicio_celda.value = titulo_general
    row_num=3
    total=0
    for objetocab in pedidodecompracab :
        ...
        for objetodet in pedidodecompradet:
            ...
            sheet.cell(row=row_num, column=2, value=objetocab.nropedido)
...
def pedidodecompracabinfpdf(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    pedidodecompracab_infpdftit(c,680,0)
    pedidodecompracab_infpdfdet(pedidodecompracab ,c,620)
    c.save()
    buf.seek(0)
...
def pedidodecompracab_infpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME PEDIDO DE COMPRA ")
        pedidodecompracab_infpdfcabdet(c,620)


def pedidodecompracab_infpdfcabdet(c,y):
...
def pedidodecompracab_infpdfdet(pedidodecompracab ,c,y):
        total=0
        detmax=25
        nropag=1
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                pedidodecompracab_infpdftit(c, 680, nropag)
                fila_y=620
...
def pedidodecompradetinf_listar(request):
    idproveedor = request.POST['idproveedor']
    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    tipodoc = request.POST['tipodoc']
    ...
    datos = []
    for objetocab in pedidodecompracab:
        idicompra =objetocab.idpedidocompcab
        pedidodecompradet = Pedidocompdet.objects.filter(idpedidocompcab =idicompra).order_by('orden')
        for objetodet in pedidodecompradet:
             datos.append({
            'fecha': objetocab.fecha.strftime("%d/%m/%Y"),
            'nropedido': objetocab.nropedido,
...
def pedidodecompradetinfpdf(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    ordedecompradetinfpdftit(c,680,0)
    ordedecompradetinfpdfdet(pedidodecompracab ,c,620)
    c.save()
    buf.seek(0)
...
def ordedecompradetinfpdftit(c,y,npag):
        ahora = time.strftime("%c")
        fechahora = time.strftime("%c")
        fecha = time.strftime("%x")
        print(npag)
        ...
        c.setFillColor(colors.black)
        c.setFont("Helvetica", 22)  # Puedes ajustar la fuente y el tamaño según tus necesidades
        c.drawString(lx -10, hi-20,"INF. PEDIDO  DE COMPRA DETALLE ")
        ordedecompradetinfcabdet(c,620)


def ordedecompradetinfcabdet(c,y):
...
def ordedecompradetinfpdfdet(pedidodecompracab ,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir coordenadas y dimensiones de la tabla
        ...
        for objetocab in pedidodecompracab :
            ...
            if pedidodecompradet.exists():
                ...
                fila_x=fila_x+50
                celda(c, objetocab.nropedido, fila_x, fila_y, 10, alto_fila,"t", "l", 20, 0)
                fila_x=fila_x+60
                ...
                for i, fila in enumerate(data):
                    ...
                    if e > detmax:
                        ...
                        nropag = nropag+1
                        e=0
                        ordedecompradetinfpdftit(c, 680, nropag)
                        fila_y=620
