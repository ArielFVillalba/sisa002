...
def compracabinf_listar(request):
...
def compracabinf(request):
    if not request.user.is_authenticated:
        return redirect('login')

    return render(request, 'comprasinf/lstcompras.html')

def compracabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    # -----------------------------------------------
    ...
    writer.writerow(["FECHA", "FACTURA", "PROVEEDOR", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nrofactura, fila.proveedor, fila.tipodoc, fila.total])
    return response
...
def compracabinfexcel(request):
    ...
    print(idproveedor+fechaini+fechafin+tipodoc)
    compracab = Compracab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idproveedor == '':
        idproveedor = 0
    if compracab.exists() and int(idproveedor) >= 1:
    ...
    headers=["FECHA", "FACTURA", "PROVEEDOR", "TIPODOC","TOTAL"]
    ...
    titulo_general = "LISTADO COMPRAS"
    ...
    for row_num, fila in enumerate(data, 3):
        ...
        sheet.cell(row=row_num, column=2, value=fila.nrofactura)
...
def compracabinfpdf(request):
    ...
    compracabinfpdftit(c,680,0)
    compracabinfpdfdet(compracab,c,620)
    c.save()
    buf.seek(0)
...
def compracabinfpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME COMPRA ")
        compracabinfpdfcabdet(c,620)


def compracabinfpdfcabdet(c,y):
...
def compracabinfpdfdet(compracab,c,y):
        total=0
        detmax=25
        nropag=1
        #data = [["FECHA", "FACTURA", "PROVEEDOR", "TIPODOC","TOTAL"]]
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                compracabinfpdftit(c, 680, nropag)
                fila_y=620
...
def compradetinfcabdet(c,y):
    ...
    fila_y = y  # Coordenada y de la esquina superior izquierda de la tabla
    alto_fila = 20  # Alto de cada fila de la tabla
    ancho_col = [30, 50, 160, 40, 60, 30, 100]  # Ancho de cada columna de la tabla 100 3.5 cm 20 7 cm
    alto_fila = 20  # Alto de cada fila de la tabla
