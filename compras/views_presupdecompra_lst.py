...
def presupdecompracabinf_listar(request):
    ...
    idempresa = desencriptar_datos(idempresa, skey)
    idsucursal = desencriptar_datos(idsucursal, skey)
    presupdecompracab = Presupuestocompcab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)

    if idproveedor == '':
        idproveedor = 0
    if presupdecompracab.exists() and int(idproveedor) >= 1:
        presupdecompracab   = presupdecompracab .filter(idproveedor_id=idproveedor,idempresa=didempresa,idsucursal=didsucursal)
    if presupdecompracab .exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        presupdecompracab   = presupdecompracab .filter(fecha__gte=fecha_inicio,idempresa=didempresa,idsucursal=didsucursal)
    if presupdecompracab .exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        presupdecompracab   = presupdecompracab .filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    if presupdecompracab .exists() and tipodoc:
        presupdecompracab   = presupdecompracab .filter(tipodoc=tipodoc,idempresa=didempresa,idsucursal=didsucursal)
...
def presupdecompracabinfpdf(request):
    ...
    presupdecompracab   = Presupuestocompcab.objects.all(idempresa=didempresa,idsucursal=didsucursal)
    if idproveedor == '':
        idproveedor = 0
    if presupdecompracab .exists() and int(idproveedor) >= 1:
        presupdecompracab   = presupdecompracab .filter(idproveedor_id=idproveedor,idempresa=didempresa,idsucursal=didsucursal)
