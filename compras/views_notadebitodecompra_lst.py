...
def notadebdecompracabinf_listar(request):
    ...
    notadebdecompracab = Notadebitocompcab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idproveedor == '':
        idproveedor = 0
    if notadebdecompracab.exists() and int(idproveedor) >= 1:
        notadebdecompracab=notadebdecompracab.filter(idproveedor_id=idproveedor,idempresa=didempresa,idsucursal=didsucursal)
    if notadebdecompracab.exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        notadebdecompracab= notadebdecompracab.filter(fecha__gte=fecha_inicio,idempresa=didempresa,idsucursal=didsucursal)
    if notadebdecompracab.exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        notadebdecompracab= notadebdecompracab.filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    if notadebdecompracab.exists() and tipodoc:
        notadebdecompracab=notadebdecompracab.filter(tipodoc=tipodoc,idempresa=didempresa,idsucursal=didsucursal)

    datos = []
    for objeto in notadebdecompracab :
        datos.append({
    ...
    total = sum([objeto.total * objeto.total for objeto in notadebdecompracab ])
    Response = JsonResponse({'datos': datos})
    return Response

def notadebdecompracab_inf(request):
    if not request.user.is_authenticated:
        return redirect('login')

    return render(request, 'comprasinf/lstnotadebitocompra.html')

def notadebdecompracabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    objetos=notadebdecompracab 

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "LISTADO NOTA DEBITO DE COMPRA", "",""])

    writer.writerow(["FECHA", "NRO NOTA", "PROVEEDOR", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nrodebito, fila.proveedor, fila.tipodoc, fila.total])
    return response

def notadebdecompradetinfcsv(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "INFORME DE NOTA DEBITO DE COMPRA","","","" ,"","","",""])

    writer.writerow(["FECHA", "NRO NOTA", "PROVEEDOR","ORDEN","CODIGO","DESCRIPCION" ,"CANTIDAD","PRECIO","IVA","SUBTOTAL"])
    total=0
    for objetocab in notadebdecompracab :
        didnotadebitocompcab = objetocab.idnotadebitocompcab
        notadebdecompradet = Notadebitocompdet.objects.filter(idnotadebitocompcab=didnotadebitocompcab).order_by('orden')

        for objetodet in notadebdecompradet:
...
def notadebdecompracabinfexcel(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    sheet = wb.active
    sheet.title = "LISTADO NOTA DEBITO DE  COMPRAS"
    ...
    titulo_general = "LISTADO NOTA DEBITO DE COMPRAS"
...
def notadebdecompradetinfexcel(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    # Crea una hoja de cálculo en el libro de trabajo
    sheet = wb.active
    sheet.title = "LISTADO NOTA DEBITO DE  COMPRAS DETALLE"
    ...
    inicio_celda.alignment = Alignment(horizontal='center')
    # Establece el valor del título general en la celda fusionada
    titulo_general = "LISTADO NOTA DE DEBITO DE COMPRAS DETALLADO  "
    inicio_celda.value = titulo_general
    row_num=3
    total=0
    for objetocab in notadebdecompracab:
...
def notadebdecompracabinfpdf(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    notadebdecompracab_infpdftit(c,680,0)
    notadebdecompracab_infpdfdet(notadebdecompracab ,c,620)
    c.save()
    buf.seek(0)
...
def notadebdecompracab_infpdftit(c,y,npag):
        ...
        c.drawString(lx+20 , hi-20,"INFORME NOTA DEBITO DE COMPRA ")
        notadebdecompracab_infpdfcabdet(c,620)


def notadebdecompracab_infpdfcabdet(c,y):
...
def notadebdecompracab_infpdfdet(notadebdecompracab ,c,y):
        total=0
        detmax=25
        nropag=1
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                notadebdecompracab_infpdftit(c, 680, nropag)
                fila_y=620
...
def notadebdecompradetinf_listar(request):
    idproveedor = request.POST['idproveedor']
    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    tipodoc = request.POST['tipodoc']
...
def notadebdecompradetinfpdf(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    notadebdecompradetinfpdftit(c,680,0)
    notadebdecompradetinfpdfdet(notadebdecompracab ,c,620)
    c.save()
    buf.seek(0)
...
def notadebdecompradetinfpdftit(c,y,npag):
        ahora = time.strftime("%c")
        fechahora = time.strftime("%c")
        fecha = time.strftime("%x")
        print(npag)
        ...
        c.setFillColor(colors.black)
        c.setFont("Helvetica", 22)  # Puedes ajustar la fuente y el tamaño según tus necesidades
        c.drawString(lx -50, hi-20,"INF. NOTA DEBITO DE COMPRA DETALLE ")
        notadebdecompradetinfcabdet(c,620)


def notadebdecompradetinfcabdet(c,y):
...
def notadebdecompradetinfpdfdet(notadebdecompracab ,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir coordenadas y dimensiones de la tabla
            ...
            if notadebdecompradet.exists():
                ...
                for i, fila in enumerate(data):
                    ...
                    if e > detmax:
                        ...
                        nropag = nropag+1
                        e=0
                        notadebdecompradetinfpdftit(c, 680, nropag)
                        fila_y=620
