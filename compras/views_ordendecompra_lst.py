...
def ordendecompracabinf_listar(request):
    ...
    ordendecompracab = Ordencompcab.objects.filter(idempresa=didempresa,idsucursal=didsucursal)
    if idproveedor == '':
        idproveedor = 0
    if ordendecompracab.exists() and int(idproveedor) >= 1:
        ordendecompracab   = ordendecompracab.filter(idproveedor_id=idproveedor,idempresa=didempresa,idsucursal=didsucursal)
    if ordendecompracab .exists() and fechaini:
        fecha_inicio = datetime.strptime(fechaini, '%Y-%m-%d').date()
        ordendecompracab   = ordendecompracab .filter(fecha__gte=fecha_inicio,idempresa=didempresa,idsucursal=didsucursal)
    if ordendecompracab .exists() and fechafin:
        fecha_fin = datetime.strptime(fechafin, '%Y-%m-%d').date()
        ordendecompracab   = ordendecompracab .filter(fecha__lte=fecha_fin,idempresa=didempresa,idsucursal=didsucursal)
    if ordendecompracab .exists() and tipodoc:
        ordendecompracab   = ordendecompracab .filter(tipodoc=tipodoc,idempresa=didempresa,idsucursal=didsucursal)

    datos = []
    for objeto in ordendecompracab :
        datos.append({
        'fecha': objeto.fecha.strftime("%d/%m/%Y"),
        'nroorden': objeto.nroorden,
        'proveedor': objeto.proveedor,
        'tipodoc': objeto.tipodoc,
        'total':objeto.total
    })
    total = sum([objeto.total * objeto.total for objeto in ordendecompracab ])
    Response = JsonResponse({'datos': datos})
    return Response


def ordendecompracab_inf(request):
    if not request.user.is_authenticated:
        return redirect('login')

    return render(request, 'comprasinf/lstordendecompra.html')

def ordendecompracabinfcsv(request):
    if not request.user.is_authenticated:
        return redirect('login')
    ...
    objetos=ordendecompracab 

    response = HttpResponse(content_type='text/csv')
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "ORDEN DE COMPRA", "",""])

    writer.writerow(["FECHA", "NRO ORDEN", "PROVEEDOR", "TIPODOC","TOTAL"])
    # Agrega los datos de cada fila
    for fila in objetos:
        writer.writerow([fila.fecha, fila.nroorden, fila.proveedor, fila.tipodoc, fila.total])
    return response

def ordendecompradetinfcsv(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    response['Content-Disposition'] = 'attachment; filename="datos.csv"'
    writer = csv.writer(response, delimiter=';')  # Establece el separador como ";"
    writer.writerow(["", "", "INFORME DE ORDEN DE COMPRA","","","" ,"","","",""])

    writer.writerow(["FECHA", "FACTURA", "PROVEEDOR","ORDEN","codigo","descripcion" ,"cantidad","precio","iva","subtotal"])
    total=0
    for objetocab in ordendecompracab :
        didordencompcab = objetocab.idordencompcab
        ordendecompradet = Ordencompdet.objects.filter(idordencompcab =didordencompcab).order_by('orden')

        for objetodet in ordendecompradet:
...
def ordendecompracabinfexcel(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    titulo_general = "LISTADO ORDEN DE COMPRAS"
    ...
    for row_num, fila in enumerate(data, 3):
        ...
        sheet.cell(row=row_num, column=2, value=fila.nroorden)
...
def ordendecompradetinfexcel(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    for objetocab in ordendecompracab :
        ...
        for objetodet in ordendecompradet:
            ...
            sheet.cell(row=row_num, column=2, value=objetocab.nroorden)
...
def ordendecompracabinfpdf(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    ordendecompracab_infpdftit(c,680,0)
    ordendecompracab_infpdfdet(ordendecompracab ,c,620)
    c.save()
    buf.seek(0)
...
def ordendecompracab_infpdftit(c,y,npag):
        ...
        c.drawString(lx + 20, hi-20,"INFORME ORDEN DE COMPRA ")
        ordendecompracab_infpdfcabdet(c,620)


def ordendecompracab_infpdfcabdet(c,y):
...
def ordendecompracab_infpdfdet(ordendecompracab ,c,y):
        total=0
        detmax=25
        nropag=1
       ...
       # Definir estilos de la tabla
        ...
        for i, fila in enumerate(data):
            if e > detmax:
                ...
                nropag = nropag+1
                e=0
                ordendecompracab_infpdftit(c, 680, nropag)
                fila_y=620
...
def ordendecompradetinf_listar(request):
    idproveedor = request.POST['idproveedor']
    fechaini = request.POST['fechaini']
    fechafin = request.POST['fechafin']
    tipodoc = request.POST['tipodoc']
    ...
    datos = []
    for objetocab in ordendecompracab:
        idicompra =objetocab.idordencompcab
        ordendecompradet = Ordencompdet.objects.filter(idordencompcab =idicompra).order_by('orden')
        for objetodet in ordendecompradet:
             datos.append({
            'fecha': objetocab.fecha.strftime("%d/%m/%Y"),
            'nroorden': objetocab.nroorden,
...
def ordendecompradetinfpdf(request):
    # -----------------------------------------------
    idproveedor = request.GET.get('idproveedor', '')
    fechaini = request.GET.get('fechaini', '')
    fechafin = request.GET.get('fechafin', '')
    ...
    ordedecompradetinfpdfdet(ordendecompracab ,c,620)
    c.save()
    buf.seek(0)
...
def ordedecompradetinfpdfdet(ordendecompracab ,c,y):
        total=0
        detmax=25
        nropag=1
        # Definir coordenadas y dimensiones de la tabla
        ...
        for objetocab in ordendecompracab :
            ...
            if ordendecompradet.exists():
                ...
                fila_x=fila_x+50
                celda(c, objetocab.nroorden, fila_x, fila_y, 10, alto_fila,"t", "l", 20, 0)
                fila_x=fila_x+60
